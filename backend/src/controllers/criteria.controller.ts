...
export const createCriteria = async (req: Request, res: Response) => {
  try {
    ...
    // Create criteria with icon path
...
export const updateCriteria = async (req: Request, res: Response) => {
  try {
    ...
    // Get existing criteria to check for old icon
    const existingCriteria = await CriteriaModel.findById(id);
    if (!existingCriteria) {
      return res.status(404).json({ success: false, message: "Criteria not found" });
    }
    
    // Handle icon update if a new file was uploaded
    const updateData: any = { ...validatedData };
    
    if (req.file) {
      // Delete old icon if it exists
      if (existingCriteria.icon) {
        deleteFile(existingCriteria.icon);
      }
      // Set new icon path
      updateData.icon = req.file.path;
    }
    
    const criteria = await CriteriaModel.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
...
export const deleteCriteria = async (req: Request, res: Response) => {
  try {
    ...
    const criteria = await CriteriaModel.findById(id);
    if (!criteria) {
      return res.status(404).json({ success: false, message: "Criteria not found" });
    }
